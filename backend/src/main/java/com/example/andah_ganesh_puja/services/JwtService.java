package com.example.andah_ganesh_puja.services;
...
public class JwtService {
    ...
    public String generateRefreshToken(ProfileEntity profileEntity) {
        Instant now = Instant.now();
        Instant expiry = now.plusMillis(Long.parseLong(refreshTokenExpiration));

        return Jwts.builder()
                .subject(profileEntity.getId().toString())
    ...
    public String generateAccessToken(ProfileEntity profileEntity, Long sessionId) { // Add sessionId param
        Instant now = Instant.now();
        Instant expiry = now.plusMillis(Long.parseLong(accessTokenExpiration));

        return Jwts.builder()
                .subject(profileEntity.getId().toString())
                .claim("email", profileEntity.getEmail())
                .claim("sessionId", sessionId) // Store the DB ID here
                .claim("roles", profileEntity.getRoles().toString())
                .issuedAt(Date.from(now))
                .expiration(Date.from(expiry))
                .signWith(getSecretKey())
                .compact();
